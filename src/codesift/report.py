...
def analyse(cfg: Config, models: list[str]) -> Assessment:
    ...
    def scores():
        ...
        quality seen, the most a below-floor model can score is 0.6 x (floor - 0.1),
        which is below the 0.6 x floor that every above-floor model is already
        ...
        comp = {m: round(0.6 * q[m] + (0.4 * speed[m] if over[m] else 0.0), 1)
                for m in elig}
...
def build(cfg: Config, models: list[str]) -> str:
    ...
    the wrong result and the model gets another turn. The remainder score 60% quality, taken
    as the hard-set pass rate, and 40% speed. Speed is the modelled time to complete one representative
